...
export function remarkLinksExtractor(options = {}) {
	...
	return function (tree, file) {
		...
		visit(
			tree,
			['heading', 'html', 'link', 'linkReference', 'mdxJsxFlowElement', 'mdxJsxTextElement'],
			(node) => {
				switch (node.type) {
					case 'heading':
...
function getSlugFromFile(file, filePath, useAstroSlug) {
	if (useAstroSlug) {
		const astroData = /** @type {{ frontmatter?: { slug?: unknown } } | undefined} */ (
			file.data?.astro
		);
		const slug =
			typeof astroData?.frontmatter?.slug === 'string' ? astroData.frontmatter.slug : undefined;
